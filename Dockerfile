...
RUN rm ./target/release/deps/bulletproofs_client* || true
RUN cargo build --release
...
COPY --from=builder /app/target/release/bulletproofs_client /app/bulletproofs_client

# Copy the wait-for-server.sh script
